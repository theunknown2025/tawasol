...
import { ChevronLeft, ChevronRight, Database, Pencil, Trash2 } from "lucide-react";
...
import { deleteBarometreCooperative, type BarometreCooperative } from "./barometreCooperativesApi";
...
export default function BarometreDatabaseAccordion({
  ...
  useEffect(() => {
    const maxIdx = Math.max(0, Math.ceil(cooperatives.length / pageSize) - 1);
    setPageIndex((p) => Math.min(p, maxIdx));
  }, [cooperatives.length, pageSize]);

  useEffect(() => {
    setExpandedId((prev) => {
      if (!prev) return null;
      return cooperatives.some((c) => c.id === prev) ? prev : null;
    });
  }, [cooperatives]);
                                  ...
                                  <div className="space-y-3 min-w-0">
                                    <div className="flex flex-wrap items-center gap-2">
                                      ...
                                      {coop.isPublished ? (
                                        ...
                                        <Badge variant="outline">Non publiée</Badge>
                                      )}
                                    </div>
                                    {coop.activite ? (
