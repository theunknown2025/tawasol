...
type EquipeManagerProps = {
  value: EquipeContent;
  onChange: (next: EquipeContent) => void;
};

export function EquipeManager({ value, onChange }: EquipeManagerProps) {
  ...
  const handleFile = async (index: number, file: File | undefined) => {
    ...
    try {
      const { url, usedFallback } = await uploadLandingPageImage(file, "equipe");
            ...
            <Card key={m.id} className="overflow-hidden shadow-sm">
              ...
              <CardContent className="space-y-4 pt-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor={`equipe-name-${m.id}`}>Nom complet</Label>
                    <Input
                      id={`equipe-name-${m.id}`}
                      value={m.fullName}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor={`equipe-pos-${m.id}`}>Poste / fonction</Label>
                    <Input
                      id={`equipe-pos-${m.id}`}
                ...
                <div className="space-y-2">
                  <Label htmlFor={`equipe-bio-${m.id}`}>
                    Bio (max. {EQUIPE_BIO_MAX_CHARS} caractères — aperçu au survol)
                  </Label>
                  <Textarea
                    id={`equipe-bio-${m.id}`}
                    value={m.bio}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor={`equipe-li-${m.id}`}>LinkedIn (URL)</Label>
                    <Input
                      id={`equipe-li-${m.id}`}
                      value={m.linkedinUrl}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor={`equipe-mail-${m.id}`}>E-mail</Label>
                    <Input
                      id={`equipe-mail-${m.id}`}
                      type="email"
                      value={m.email}
