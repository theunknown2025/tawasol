...
import type { HeroSectionContent, RemessEnChiffresContent } from "../types";
import { slideBackgroundStyle } from "../types";
import { cn } from "@/lib/utils";
import { HeroChiffresOverlay } from "./HeroChiffresOverlay";

type HeroSectionProps = {
  content: HeroSectionContent;
  className?: string;
  /** Chiffres clés affichés en overlay bas du hero (max. 6). */
  chiffres?: RemessEnChiffresContent | null;
  showChiffres?: boolean;
};
...
export function HeroSection({
  ...
  const hasChiffres =
    showChiffres && chiffres != null && chiffres.stats.length > 0;
  ...
  return (
    <section
      className={cn(
        ...
        /* Réserve l’espace sous le hero pour les cartes qui dépassent de ~50 %. */
        hasChiffres && "mb-16 sm:mb-20 md:mb-24",
        className,
      )}
    ...
    >
      {/* Zone visuelle du hero : les chiffres s’ancrent ici, pas dans un padding vide. */}
      <div className="relative min-h-[70vh] w-full overflow-visible">
        <div className="absolute inset-0 overflow-hidden">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  hasChiffres && "pb-28 sm:pb-32",
                  active
                    ? "z-[1] pointer-events-auto opacity-100"
          ...
          <div
            className="pointer-events-none absolute inset-x-0 bottom-0 z-[5] h-[clamp(3rem,8vw,6rem)] bg-gradient-to-t from-[#5f2314]/50 via-[#5f2314]/20 to-transparent"
            aria-hidden
          />
          ...
          {n > 1 && (
            <div
              className={cn(
                ...
                hasChiffres ? "bottom-16 sm:bottom-20" : "bottom-6",
              )}
              aria-hidden
            >
              {slides.map((s, i) => (
        ...
        {hasChiffres && chiffres ? <HeroChiffresOverlay content={chiffres} /> : null}
      </div>
    </section>
  );
}
