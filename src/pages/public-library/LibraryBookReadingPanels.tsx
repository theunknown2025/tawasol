...
export const libraryBookReviewsQueryKey = (bookId: string) => ["public-library-reviews", bookId] as const;
...
type LibraryBookPdfPanelProps = {
  bookId: string;
  title: string;
  pdfUrl: string;
  showShare?: boolean;
  /** Hauteur minimale de la zone PDF (aperçu pleine page vs dialogue). */
  minHeightClass?: string;
};

export function LibraryBookPdfPanel({
  bookId,
  title,
  pdfUrl,
  showShare = true,
  minHeightClass = "min-h-[50vh] lg:min-h-[70vh]",
}: LibraryBookPdfPanelProps) {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-2 sm:px-5">
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Document</span>
        <div className="flex flex-wrap items-center gap-2">
          {showShare ? (
            <ShareResourceMenu
              title={title}
              url={buildArticleShareUrl(bookId)}
              variant="outline"
              size="sm"
            />
          ) : null}
          {trimmed ? (
            <Button
              ...
              className="gap-2"
              onClick={() => {
                void incrementLibraryBookDownloads(bookId);
      ...
      <div className={cn("min-h-0 flex-1 bg-muted/30", minHeightClass)}>
        {trimmed ? (
          <iframe title={title} src={trimmed} className={cn("h-full w-full border-0", minHeightClass)} />
        ) : (
...
type LibraryBookReviewsPanelProps = {
  bookId: string;
  description?: string;
  /** Si false, les requêtes avis restent désactivées (ex. dialogue fermé). */
  enabled?: boolean;
  className?: string;
};

export function LibraryBookReviewsPanel({
  bookId,
  description,
  enabled = true,
  className,
}: LibraryBookReviewsPanelProps) {
  const { user } = useAuth();
  ...
  const { data: reviews = [], isLoading: reviewsLoading } = useQuery({
    queryKey: libraryBookReviewsQueryKey(bookId),
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setComment("");
      setRating(0);
      void queryClient.invalidateQueries({ queryKey: [...libraryBookReviewsQueryKey(bookId)] });
    },
    onError: (e: Error) => {
  ...
  return (
    <div className={cn("space-y-5 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5", className)}>
      {description?.trim() ? (
        <div>
      ...
      <div className="border-t border-border pt-4">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Votre avis</h3>
        {!user ? (
          ...
          <form className="space-y-3" onSubmit={handleSubmitReview}>
            ...
            <div className="space-y-2">
              <Label htmlFor={`rev-comment-${bookId}`}>Commentaire</Label>
              <Textarea
                id={`rev-comment-${bookId}`}
                value={comment}
